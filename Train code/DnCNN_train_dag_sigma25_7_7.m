function [net,stats] = DnCNN_train_dag_sigma25_7_7(net, varargin)
...
opts.gpus = [];
opts.numEpochs = 300 ;
opts.modelName   = 'model_sigma25_7_7';
...
% Ysize = ceil(size(label,1)/2); % +3;
% Xsize = ceil(size(label,2)/2); % +3;  
% input1 = zeros(Ysize,Xsize,4,'single');
% label1 = zeros(Ysize,Xsize,4,'single');
% for iii = 1 : size(label,4)
%     [label1(:,:,1,iii), label1(:,:,2,iii), label1(:,:,3,iii), label1(:,:,4,iii)] = dwt2(double(label(:,:,1,iii)), 'haar');
%     [input1(:,:,1,iii), input1(:,:,2,iii), input1(:,:,3,iii), input1(:,:,4,iii)] = dwt2(double(label(:,:,1,iii)), 'haar');
%     label1(:,:,1,iii) = noiselevel/255*randn(size(label1(:,:,1,iii)),'single');
%     label1(:,:,2,iii) = noiselevel/255*randn(size(label1(:,:,2,iii)),'single');
%     label1(:,:,3,iii) = noiselevel/255*randn(size(label1(:,:,3,iii)),'single');
%     label1(:,:,4,iii) = noiselevel/255*randn(size(label1(:,:,4,iii)),'single');
%     input1(:,:,1,iii)      = input1(:,:,1,iii) + label1(:,:,1,iii);
%     input1(:,:,2,iii)      = input1(:,:,2,iii) + label1(:,:,2,iii);
%     input1(:,:,3,iii)      = input1(:,:,3,iii) + label1(:,:,3,iii);
%     input1(:,:,4,iii)      = input1(:,:,4,iii) + label1(:,:,4,iii);
% end
%%
label  = imdb.labels(2:end,2:end,:,batch);  %label      = imdb.labels(:,:,:,batch);
label  = data_augmentation(label,randi(8));% input      = label + noiselevel/255*randn(size(label),'single');  % add AWGN with noise level noiselevel
input1 = zeros(size(label,1)+1, size(label,2)+1, 5, size(label,4), 'single');
label1 = noiselevel/255*randn(size(input1),'single');
[ld,hd,lr,hr] = wfilters('bior1.1');
for iii = 1 : size(label,4)
    input1(2:end,2:end,1,iii) = label(:,:,1,iii);    
    [Wave_S,Wave_HW,Wave_WH,Wave_WW,etl] = ocwt2dliu1(label(:,:,1,iii),ld,hd,1);
    input1(:,:,2,iii) = Wave_S;
    input1(:,:,3,iii) = Wave_HW{1,1}; 
    input1(:,:,4,iii) = Wave_WH{1,1}; 
    input1(:,:,5,iii) = Wave_WW{1,1};
end
input1 = input1 + label1;
%%
input1      = gpuArray(input1);
label1      = gpuArray(label1);
inputs2    = {'input', input1, 'label', label1} ;
